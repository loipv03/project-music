...
import { getTop100s } from "../../redux/slice/top100";
...
const Top100 = () => {
  ...
  const state = useSelector(({ top100 }: any) => top100.top100);

  useEffect(() => {
    AppDispatch(getTop100s());
  }, []);
  ...
  return (
    <div className={cx("top100")}>
      ...
      {state?.map((top100: any) => (
        <div key={top100?.title} className={cx("content")}>
          <div className={cx("title")}>{top100?.title}</div>
          <div className={cx("content_wrap")}>
            {top100?.items?.map((item: any, index: number) => {
              if (index <= 4) {
                return (
                  <Link key={item?.encodeId} to={item?.link.split(".")[0]}>
                    <div className={cx("content_item")}>
                      ...
                      <div className={cx("title_item")}>{item?.title}</div>
                      <div className={cx("artistsNames")}>
                        {item?.artists?.map(
